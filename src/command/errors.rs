...
#[derive(Debug)]
pub struct CommandExecutionError(pub Verb, pub String);
...
mod tests {
    ...
    fn test_fmt_execution_error_no_message() {
        let err = CommandExecutionError(Verb::USER, "".to_string());
        let s = format!("{}", err);
    ...
    fn test_fmt_execution_error_message() {
        let err = CommandExecutionError(Verb::USER, "foo".to_string());
        let s = format!("{}", err);
